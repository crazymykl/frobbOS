...
target/kernel.bin: target/multiboot_header.o target/boot.o src/asm/linker.ld cargo
	$(LD) -n -o target/kernel.bin -T src/asm/linker.ld target/multiboot_header.o target/boot.o target/x86_64-unknown-frobbos-gnu/release/libfrobbos.a
...
target/libcore/target/x86_64-unknown-frobbos-gnu/libcore.rlib: target/libcore
	cp x86_64-unknown-frobbos-gnu.json target/libcore
	cd target/libcore && cargo build --release --features disable_float --target=x86_64-unknown-frobbos-gnu.json

cargo: target/libcore/target/x86_64-unknown-frobbos-gnu/libcore.rlib
	RUSTFLAGS="-L target/libcore/target/x86_64-unknown-frobbos-gnu/release" cargo build --release --target x86_64-unknown-frobbos-gnu.json
